...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # Final cost matrix
        #（200，6）
        #box=5,class=1,giou=2
        C = self.cost_class * cost_class #(bs*num_queries,bs)
        # 第一个查询，这6个类的损失。第二个查询，这6个类的损失。。。
        C = C.view(bs, num_queries, -1) #(bs,num_queries,bs).本来是92个obj。只关注tgt给出的那6个
        #@sizes = [len(v) for v in targets]
        #sizes = [torch.full(len(targets),1,dtype = torch.int64,device=outputs.device)]
...
def build_matcher(args):
    ...
    return HungarianMatcher(cost_class=1)
